package com.gxkzw.gx.web.login;
...
import com.gxkzw.gx.common.model.Session;
...
public class LoginService {
	...
	public static final String sessionIdName = "gxkzwId";
		
	/**
	 * 
	 * @param userName !注意:此处的userName是代表邮箱或身份证号
	 * @param password
	 * @param keepLogin
	 * @param loginIp
	 * @return
	...
	public Ret login(String userName,String password,boolean keepLogin,String loginIp) {
		...
		Account loginAccount = dao.findFirst("select * from account where userName = ? or idNumber = ? limit 1",userName,userName);
		if(loginAccount == null) {
			return Ret.fail("msg", "邮箱或身份证不正确");
		}
...
//			createLoginLog(loginAccount.getId(), loginIp);
	...
	public Ret sendRetrievePasswordAuthEmail(String userName) {
		...
		String title = "[广西考证网]密码找回邮件";
		String content = "在浏览器地址栏里输入并访问下面链接即可重置密码：\n\n"
				+ " "+baseUrl+"/login/retrievePassword?authCode="
				+ authCode;
